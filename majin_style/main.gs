 ...
 * 「テスト生成」専用。Gemini の構造化出力で JSON を生成し、スライド化します。
 */
...
function mainTest(prompt, generateOption) {
  try {
    ...
    Logger.log(`テスト生成完了: ${presentationUrl}`);
    return presentationUrl;
  } catch (error) {
    Logger.log(`テスト生成エラー: ${error.message}`);
